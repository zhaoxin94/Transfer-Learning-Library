...
def main(args: argparse.Namespace):
    ...
    for epoch in range(args.epochs):
        # train for one epoch
        train(train_source_iter, train_target_iter, G, F1, F2, GRL, optimizer_g, optimizer_f, epoch, args)
...
def train(train_source_iter: ForeverDataIterator, train_target_iter: ForeverDataIterator,
          G: nn.Module, F1: ImageClassifierHead, F2: ImageClassifierHead, GRL: GradientReverseLayer,
    ...
    for i in range(args.iters_per_epoch):
        ...
        assert x_s.requires_grad is False
        assert x_t.requires_grad is False

        # train all networks to minimize loss on source domain
